...
-- nomor 3
SELECT ProductName,productScale FROM products p1 WHERE productName IN ( SELECT productName from products where productName like "%Ford%"); 
...
FROM customers
WHERE  CHAR_LENGTH(Country) =(SELECT MAX(CHAR_LENGTH(Country)) FROM customers) 
